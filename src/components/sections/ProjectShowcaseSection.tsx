...
const SHOWCASE = [
  {
    title: "Scripting",
    description:
      "UI systems, combat, movement, and gameplay logic built in Roblox Studio.",
    href: "#scripting",
    icon: Code2,
  ...
  },
  {
    title: "UI Systems",
    description:
      "Menus, shops, inventory logic, and polished in-game interfaces.",
    href: "#scripting",
    icon: Layout,
    accent: "cyan" as const,
...
function ShowcaseCard({ item }: { item: (typeof SHOWCASE)[number] }) {
  return (
    <article className={CARD_HEIGHT}>
      <AnimatedBorder accent="purple" glow={false} className="h-full">
        <a
          href={item.href}
          className={cn(
            "relative flex flex-col overflow-hidden rounded-2xl p-5 transition-colors hover:bg-white/[0.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)]/40",
            CARD_HEIGHT
          )}
        >
          <div
          ...
          <p className="mt-1.5 line-clamp-3 flex-1 text-xs leading-relaxed text-[var(--muted)] sm:text-sm">
            {item.description}
          </p>
